...
import { X, CalendarPlus, BookOpen, Check, Clock, Target, Loader2, Users, UserPlus, Trash2 } from "lucide-react";
          ...
          {!isEdit && (
            <div className="space-y-3">
              <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">
                1. Select Quiz {errors.quiz && <span className="text-red-500 ml-2">{errors.quiz}</span>}
              </label>
              {initialQuizId ? (
                <div className="flex items-center gap-3 p-4 rounded-2xl border border-border bg-secondary/30">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-sm">
                      {quizzes.find(q => q.id === initialQuizId)?.title ?? "Loading..."}
                    </p>
                  </div>
                </div>
              ) : quizzes.length === 0 ? (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">{quiz._count.questions} questions</p>
                        </div>
                      </div>
                      {selectedQuizId === quiz.id && <Check size={16} className="text-primary shrink-0" />}
                    </div>
                  ))}
                </div>
              )}
